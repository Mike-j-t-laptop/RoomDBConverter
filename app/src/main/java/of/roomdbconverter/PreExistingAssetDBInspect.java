...
import static of.roomdbconverter.SQLiteConstants.*;

public class PreExistingAssetDBInspect {

    //Prefix applied to database name used for inspection
    public static final String INSPECTION_PREFIX = "inspect_";
    ...
    private String[] mAssetSubDirectories;
    ...
    public PreExistingAssetDBInspect(Context context, String databaseName, String assetFileName, String[] assetSubDirectories) {
        mContext = context;
        mDatabaseName = databaseName;
        mAssetSubDirectories = assetSubDirectories;
        mDatabaseDiskSize = 0;
        mAssetFileName = assetFileName;
        if (assetFileName == null || assetFileName.length() < 0) {
            mAssetFileName = assetFileName;
        }
        mAssetPath = AssetEntry.buildAssetPath(mAssetSubDirectories,mAssetFileName);
        mInspectDBName = INSPECTION_PREFIX + mDatabaseName;
        mDatabasePath = context.getDatabasePath(mInspectDBName).toString();
        mInspectDB = createInspectionDBFromAssetDB(context,mDatabasePath,mAssetPath);
    ...
    public PreExistingAssetDBInspect(Context context, String databaseName, String[] assetSubDirectories) {
        this(context,databaseName,null,assetSubDirectories);
    }

    public PreExistingAssetDBInspect(Context context, String databaseName) {
        this(context, databaseName, null, null);
    }

    public PreExistingAssetDBInspect(Context context, String databaseName, String assetFileName) {
        this(context,databaseName,assetFileName, null);
    }
    ...
    /**
     * Re-copy if Inspection Database
     */
    public void recopyInspectionDatabase() {
        createInspectionDBFromAssetDB(mContext,mDatabasePath,mAssetPath);
    }
    ...
    public void setAssetFileName(String assetFileName) {
        this.mAssetFileName = mAssetFileName;
    }
    ...
    /**
     * Create the Inspection Database from the Asset DB
     * @param context       The context used for obtaining the asset
     * @param databasePath  The path to the database
     * @param assetPath     The path (within AssetManager) of the asset
     * @return              The copied database
     */
    private SQLiteDatabase createInspectionDBFromAssetDB(Context context, String databasePath, String assetPath) {
        checkDBExists(context,databasePath,true); //Always recopy the DB from the assets
        if (!AssetEntry.checkAssetDBValidity(context,assetPath)) {
            return null;
        }
        AssetEntry.copyAsset(context,assetPath, databasePath);
        this.mDatabaseDiskSize = AssetEntry.getLastAssetSize();
        return SQLiteDatabase.openDatabase(mDatabasePath,null,SQLiteDatabase.OPEN_READWRITE);
    }

    /**
     * Check if the database Directory exists, with force delete option.
     * NOTE will create the databases directory if it doesn't exist
     * @param context           The context for determining the location of the database
     * @param databasePath      The path to the database
     * @param deleteIfExists    true if to delete the database if it exists
     * @return                  false if the database does not exist
     */
    public static boolean checkDBExists(Context context, String databasePath, boolean deleteIfExists) {
        //Log.d("CHECKDBEXISTS","Checking if " + databasePath + " exists.");
        boolean rv = false;
        File db = new File(databasePath);
        if (deleteIfExists && db.exists()) {
            db.delete();
            //Log.d("CHECKDBEXISTS","Force deleting " + databasePath);
            rv = false;
        } else if (db.exists()) return true;
        //Log.d("CHECKDBEXISTS","Database " + databasePath + " does not exist or was deleted.");
        File dbdirectory = db.getParentFile();
        if (!dbdirectory.exists()) {
            //Log.d("CHECKDBEXISTS","Build directories for " + databasePath);
            dbdirectory.mkdirs();
        }
        return rv;
    }
